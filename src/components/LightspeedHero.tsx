...
export function LightspeedHero() {
  ...
  useEffect(() => {
    ...
    style.textContent = `
      /* How much higher the stem's top moves when scaled (160px * 0.15) */
      ...
      #foot {
        transform-origin: 50% 100%; /* bottom center so it collapses inward */
        transition: transform .5s cubic-bezier(.4,0,.2,1);
      }

      /* The entire tower head moves as one so it always sits on the stem's top */
      #tower {
      ...
      .hover-campanile-container:hover #foot {
        transform: scaleX(.22) scaleY(.6); /* collapses to center, no left drift */
      }
      .hover-campanile-container:hover #tower {
        transform: translateY(calc(-1 * var(--lift))); /* ride with stem top */
      }
            ...
            >
              <div className="glyph-container" style={{ width: "100%", height: "100%" }}>
                ...
                >
                  ...
                  {/* Tower head — anchored to the stem's TOP CENTER (x=36+34/2=53, y=40) */}
                  <g id="tower" transform="translate(53,40)">
                    {/* Mask for arches (local coords) */}
                    <defs>
                    ...
                    {/* Spire above the belfry (triangle pointing up) */}
